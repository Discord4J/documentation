...
There are multiple ways of responding to a command. The simplest would be `reply()`:
...
If it takes longer than 3 seconds to gather the data and respond, use `acknowledge` and when you are ready to respond with data, use `followup`

When you use acknowledge, you have up to 15 minutes before the interaction is no longer valid.
...
public static void main(String[] args) {
    ...
    client.on(ChatInputInteractionEvent.class, event -> {
        if (event.getCommandName().equals("ping")) {
            return event.acknowledge().then(methodThatTakesALongTime(event));
        }
    }).subscribe();
}

private static Mono<MessageData> methodThatTakesALongTime(ChatInputInteractionEvent event) {
    // Do logic that takes awhile, then return
    return event.getInteractionResponse().createFollowupMessage("This took awhile!");
}
```

### Ephemeral Responses

When using `reply`/`acknowlege`/`followup` everyone in the channel can see this, not just the user that issued the command.

Since you may not always want that, Discord provides `ephemeral` versions of these response types. `ephmeral` means that only the user who issued the command will see the response.
...
:::note
If you acknowledge ephemerally, you must use an ephemeral followup.

The same goes in reverse, if you acknowledge normally, you must also follow up normally. Discord does not support mixing these response modifiers.
:::
...
client.on(ChatInputInteractionEvent.class, event -> {
    if (event.getCommandName().equals("ping")) {
        ...
        event.getInteractionResponse().editInitialResponse(WebhookMessageEditRequest.builder()
            .contentOrNull("Pong! Pong!")
            .build()
        ).subscribe();
    }
}).subscribe();
...
client.on(ChatInputInteractionEvent.class, event -> {
    if (event.getCommandName().equals("ping")) {
        ...
        event.getInteractionResponse().createFollowupMessage("PingPing!").subscribe();
        event.getInteractionResponse().createFollowupMessage("PongPong!").subscribe();
    }
}).subscribe();
```
...
```java
Mono<String> optionMono = Mono.justOrEmpty(event.getOption("OPTION_NAME_HERE")
    .flatMap(ApplicationCommandInteractionOption::getValue)
    ).map(ApplicationCommandInteractionOptionValue::asString)
    .defaultIfEmpty("YOUR DEFAULT VALUE");
```
    ...
    if (event.getCommandName().equals("greet")) {
        ...
        */
        ...
        return nameMono.flatMap(name -> {
            //Reply to the command, with the name the user supplied
            return event.reply()
            .withEphemeral(true)
...
```java
event.reply("pong!").subscribe();
event.getInteractionResponse().deleteInitialResponse().subscribe();
```

Finally, you may want to get the initial reply data:

```java
event.reply("pong!").subscribe();
MessageData message = event.getInteractionResponse().getInitialResponse().block();
```
