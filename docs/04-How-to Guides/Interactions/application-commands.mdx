...
If it takes longer than 3 seconds to gather the data and respond, use `deferReply()` and when you are ready to respond with data, use `followup`
...
When using `reply`/`deferReply`/`followup` everyone in the channel can see this, not just the user that issued the command.
